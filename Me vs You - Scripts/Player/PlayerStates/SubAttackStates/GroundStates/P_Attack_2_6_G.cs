...
public class P_Attack_2_6_G : PlayerAbilityState
{
    public P_Attack_2_6_G(Player player, PlayerStateMachine stateMachine, PlayerData playerData, string animBoolName) : base(player, stateMachine, playerData, animBoolName)
    {
    }
    ...
    public override void Enter()
    {
        ...
        player.AnimManager.PlayTargetAnimation(player.Input.combo[1].animationName[4], false, true);
    ...
    public override void LogicUpdate()
    {
        ...
        player.IngameUi.ChangeText("", "Attack 2", "", "Attack");
       ...
       // }
        ...
        if (player.canCombo && player.isGrounded && player.Input.DreieckPressed)
        {
            stateMachine.ChangeState(player.Attack_2_7_G);
        }
        if (player.canCombo && player.isGrounded && player.Input.KreisPressed)
        {
            stateMachine.ChangeState(player.Attack_4_3_G);
        }
    }
}
